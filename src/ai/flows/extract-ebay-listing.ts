...
const ExtractEbayOutputSchema = z.object({
    ...
    brand: z.string().describe("The manufacturer name ONLY (e.g., 'Panini', 'Topps', 'Upper Deck'). Do NOT include the year or subset name."),
    set: z.string().optional().describe("The specific subset or series name ONLY, NOT the brand or year. Examples: 'Young Guns', 'Prizm', 'Chrome', 'Series 2', 'O-Pee-Chee Platinum', 'Optic'. If there is no specific subset (i.e., it is just the base brand product), return 'Base'. Do NOT repeat the brand name here."),
    cardNumber: z.string().optional().describe("The card number usually found on the back, e.g., '201', 'DTA-TT', 'C-1'. Do NOT use the serial number/print run here."),
    serialNumber: z.string().optional().describe("The print run/serial number if present (e.g., '/149', '25/99'). Leave blank if not numbered."),
